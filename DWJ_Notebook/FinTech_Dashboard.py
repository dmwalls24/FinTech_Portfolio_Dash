...
header_card = html.Div(
    children=[
        ...
        html.H1(children='FinTech Portfolio', className='header-title'),
        html.P(
            children='Optimize your portfolio',
...
def generate_options_card(num_options):
    ...
    for i in range(num_options):
        ...
        if i == 0:
            ...
            value_input_card = [
                html.Div(children=' Number of Shares', className='menu-title'),
                dcc.Input(
                    id=f'initial_amt_{i}',
                    type='number',
                    placeholder=1,
...
def update_charts(jsonified_cleaned_data):
    ...
    amount_chart_figure = make_subplots(rows=2, cols=1,
                                        subplot_titles=[f'Total Portfolio Value ($)',
                                                        f'Daily % Change of Portfolio'
                                                        ],
                                        shared_xaxes='all',
    ...
    amount_chart_figure.update_layout(template='plotly_white', showlegend=True, hovermode='x unified',
                                      ...
                                      title_text=f'Overall Performance of '
                                                 f'Portfolio ({", ".join(non_index_stocks["symbol"].unique())})')
